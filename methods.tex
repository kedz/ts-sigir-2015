...
%\ckcomment{(Paragraphs in blue should replace last two paragraphs in geo relevance section)\\
...
%Extracting features for just these sentences
%would leave 
% Last sentence is unclear
%Assigning a null or default value to sentences without explicit location is 
%not a viable option as 
%the imbalance of null/default values would make it difficult for the model
%to learn meaningful weights for these features.
% also unclear
...
\subsubsection{Model}
%\ckcomment{(replace section with this)\\
...
%Given the feature representation/salience pairs, we can use
